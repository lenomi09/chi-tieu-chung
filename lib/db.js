...
async function doInit() {
  ...
  // DB được tạo trước khi có cột status (bản cũ) sẽ không có cột này — thêm vào nếu thiếu.
...
async function getExpenses() {
  ...
  const [expRs, shareRs] = await Promise.all([
    client.execute(
      'SELECT id, date, description, amount, payer_id, status FROM expenses ORDER BY date DESC, rowid DESC'
    ),
...
async function addExpense({ date, description, amount, payerId, shareMemberIds, status = 'approved' }) {
  await init();
  const id = newId();
  const statements = [
    {
      sql: 'INSERT INTO expenses (id, date, description, amount, payer_id, status) VALUES (?, ?, ?, ?, ?, ?)',
      args: [id, date, description, amount, payerId, status],
    },
...
async function getExpenseById(id) {
  ...
  const rs = await client.execute({
    sql: 'SELECT id, date, description, amount, payer_id, status FROM expenses WHERE id = ?',
    args: [id],
  });
...
async function updateExpense(id, { date, description, amount, payerId, shareMemberIds }) {
  await init();
  const statements = [
    {
      sql: 'UPDATE expenses SET date = ?, description = ?, amount = ?, payer_id = ? WHERE id = ?',
      args: [date, description, amount, payerId, id],
    },
