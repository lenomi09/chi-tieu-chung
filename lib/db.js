...
async function getExpenses() {
  ...
  const [expRs, shareRs] = await Promise.all([
    client.execute('SELECT id, date, description, amount, payer_id FROM expenses ORDER BY date DESC, rowid DESC'),
...
async function addExpense({ date, description, amount, payerId, shareMemberIds }) {
  await init();
  const id = newId();
  const statements = [
    {
      sql: 'INSERT INTO expenses (id, date, description, amount, payer_id) VALUES (?, ?, ?, ?, ?)',
      args: [id, date, description, amount, payerId],
    },
...
async function addSettlement({ date, fromId, toId, amount, status }) {
  await init();
  const id = newId();
